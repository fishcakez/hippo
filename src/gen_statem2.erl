...
start(Module, Args, Opts) ->
    gen2:start(?MODULE, nolink, Module, Args, Opts).
%%
-spec start(
...
start(ServerName, Module, Args, Opts) ->
    gen2:start(?MODULE, nolink, ServerName, Module, Args, Opts).
...
start_link(Module, Args, Opts) ->
    gen2:start(?MODULE, link, Module, Args, Opts).
%%
-spec start_link(
	ServerName :: server_name(),
	Module :: module(), Args :: term(), Opts :: [start_opt()]) ->
...
start_link(ServerName, Module, Args, Opts) ->
    gen2:start(?MODULE, link, ServerName, Module, Args, Opts).
...
stop(ServerRef, Reason, Timeout) ->
    gen2:stop(ServerRef, Reason, Timeout).
...
-spec call(
	...
	Timeout :: timeout()) ->
		  Reply :: term().
call(ServerRef, Request, infinity) ->
    try gen2:call(ServerRef, '$gen_call', Request, infinity) of
	{ok,Reply} ->
	    Reply
    catch
	Class:Reason ->
...
enter(Module, Opts, CallbackMode, State, Data, Server, Actions, Parent) ->
    ...
    Name = gen2:get_proc_name(Server),
    Debug = gen2:debug_options(Name, Opts),
...
init_it(Starter, Parent, ServerRef, Module, Args, Opts) ->
	...
	Result ->
	    init_result(Starter, Parent, ServerRef, Module, Result, Opts);
	Class:Reason ->
	    gen2:unregister_name(ServerRef),
...
format_status(
  ...
  [PDict,SysState,Parent,Debug,
   #{name := Name, postponed := P} = S]) ->
    Header = gen2:format_status_header("Status for state machine", Name),
